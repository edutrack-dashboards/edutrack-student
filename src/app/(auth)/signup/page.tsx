...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // 1. Check if this email was invited by a manager (uses SECURITY DEFINER function)
    ...
    // 3. Link auth account to existing student record (uses SECURITY DEFINER function)
    if (data.user) {
      const { error: claimError } = await supabase.rpc("claim_student_record", {
        student_email: email.trim().toLowerCase(),
        user_id: data.user.id,
      });

      if (claimError) {
        console.error("Could not link student record:", claimError.message);
      }
    }
    ...
    <div className="w-full max-w-sm">
      <div className="rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          <h1 className="text-2xl font-bold text-gray-900">EduTrack Student</h1>
